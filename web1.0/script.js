document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMacdChartSVG(series) {
        ...
        series.forEach(b => { pMin = Math.min(pMin, b.l); pMax = Math.max(pMax, b.h); [b.ma8, b.ma34].forEach(v => { if (v != null) { pMin = Math.min(pMin, v); pMax = Math.max(pMax, v); } }); });
        const pRange = (pMax - pMin) || 1;
        const py = v => kBot - (v - pMin) / pRange * (kBot - kTop);
        let mAbs = 1e-6;
        series.forEach(b => ['dif','dea','hist'].forEach(k => { if (b[k] != null) mAbs = Math.max(mAbs, Math.abs(b[k])); }));
        const my = v => mMid - (v / mAbs) * ((mBot - mTop) / 2);
        const cx = i => plotL + i * slot + slot / 2;
        const parts = [];
        ...
        // MA34 均线
        let ma = '';
        series.forEach((b, i) => { if (b.ma34 == null) return; ma += (ma ? 'L' : 'M') + cx(i).toFixed(1) + ' ' + py(b.ma34).toFixed(1) + ' '; });
        if (ma) parts.push(`<path d="${ma}" fill="none" stroke="#8b5cf6" stroke-width="1"/>`);
        let ma8 = '';
        series.forEach((b, i) => { if (b.ma8 == null) return; ma8 += (ma8 ? 'L' : 'M') + cx(i).toFixed(1) + ' ' + py(b.ma8).toFixed(1) + ' '; });
        if (ma8) parts.push(`<path d="${ma8}" fill="none" stroke="#0891b2" stroke-width="1"/>`);
        ...
        const linePath = (key, col) => {
            let dd = '';
            series.forEach((b, i) => { if (b[key] == null) return; dd += (dd ? 'L' : 'M') + cx(i).toFixed(1) + ' ' + my(b[key]).toFixed(1) + ' '; });
            return dd ? `<path d="${dd}" fill="none" stroke="${col}" stroke-width="1"/>` : '';
        };
        ...
        const ribbon = (y, colorArr, textArr, legend) => {
            let out = `<text x="2" y="${y+12.5}" font-size="9" fill="#555">${legend}</text>`;
            const segs = []; let sidx = 0;
