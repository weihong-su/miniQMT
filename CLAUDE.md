...
| | [j]-[m],[v] | 自动买入 启动(实盘)/停止/状态/日志，[v]=模拟启动(不下单) |
| 数据与配置 | [n]-[p] | Tushare / 大QMT IPC / XtTrader 通道总控 |
| | [r]-[u] | 交割单：迁移 / 历史回填 / 导入对账单 / 导出 |

> 二级页内 **`[b]` 或直接回车返回主菜单**；`[q]` 在任何页都退出。
> 各页按键不重叠（env 用 0-4 / 首页用 5-9,a-c / services 用 d-m,v / data 用 n-p,r-u），
...
对齐 QMT 官方 [xtquant/xttype.py](xtquant/xttype.py) 的字段定义。统一前 `order_id` 曾有
**5 种叫法**（订单号/委托号/订单编号/`order_id=`/`trade_id=`），且 `order_sysid` 被叫成
「系统订单号」与「订单号」撞车 —— 两者在同一行日志里并排出现过。

| 变量 | 日志写法 | 说明 |
|------|---------|------|
| `seq` | `请求序号=` | 异步下单的本地临时凭据，回调前使用 |
| `order_id` | **`委托号=`** | 全链路主键，贯穿下单→回调→成交→落库 ⭐ |
| `order_sysid` | `柜台编号=` | 交易所/柜台侧编号 |
| `traded_id` | `成交编号=` | 真实成交号(20位) |
| `trade_records.trade_id` | `流水号=` | 落库主键，值可能是占位/委托号/成交编号 |

> ⚠️ **网格路径下「流水号」== 「委托号」是设计而非 Bug**：
> `grid_trading_manager.handle_deal_callback` 写库时用的是 `trade_id=str(order_id)`。

**写新日志时必须遵守**:
- 字段一律用 `key=value`（不用 `key: value`）
- 方向用 `config.TRADE_SIDE_LABELS`（BUY→买入），状态用 `config.ORDER_STATUS_LABELS`
  并带原码，如 `委托状态=已报(50)`
- **价格必须标语义**：`委托价=`(滑点后报价) / `成交价=` / `触发价=` / `档位价=` / `均价=`
  —— 同一笔交易这几个数字并不相同(如 68.96 / 68.97 / 69.09)，只写「价格」会误导
- `[网格]` / `[网格库]` / `[网格-策略]` 是模块标记；**info/warning/error 的正文不得带内部
  函数名前缀**（`[GRID] execute_grid_trade:` 这类曾有 155 条，且出现过前缀与所在函数不符）；
  `debug` 级可保留函数名以便排障
- **不要动** `df['订单编号']` 等 DataFrame 列名、`['order_id','m_strOrderID','订单编号']`
  这类字段名探测列表 —— 它们是数据结构不是文案
- 正文中**不要出现 ` - `**（空格-连字符-空格），会污染下游日志解析的字段边界
...
| `fast` | critical | 快速验证子集（当前配置 54 个模块、1345 个用例） |

**测试统计（当前配置）**: 37组（含 `fast`）。`--all` 默认排除重复的 `fast` 组；最近一次（2026-09-19, v3.9.3）使用 Anaconda `python39` 执行 `--all-with-fast` 实测为 37组、154个模块、3248个用例，100% 通过；具体以本地运行报告为准。
