# 大QMT RPC 方案 —— 基于 xtquant_big_convert 的交易/行情后端

> 在现有「大QMT文件IPC方案」之外，新增第三种交易后端：复用开源库
> [xtquant_big_convert](https://github.com/litaolemo/xtquant_big_convert) 的 RPC 桥接，
> 把大QMT内置 Python 的交易与行情能力通过 Redis/ZMQ/MySQL 远程暴露给 miniQMT。
>
> 与文件IPC解决同一问题（大QMT 无法用 XtQuantServer/xttrader 直连），但通道从
> 「文件轮询（~1s）」升级为「RPC（Redis ~13ms / ZMQ ~0.7ms，可跨机）」，
> 并额外获得 xtdata 全套行情能力（117 个只读方法）。
>
> **本文档为落地实施计划（设计文档），非最终代码。** 编码前需先完成第九节 P0 Spike。

---

## 一、为什么要这个方案

miniQMT 现有三种交易通道，均通过 [position_manager.py](../position_manager.py) 的
`_create_qmt_trader()` 工厂按开关选择：

| 通道 | 开关 | 通道介质 | 行情能力 |
|------|------|---------|---------|
| xttrader 直连 | 默认 | miniQMT XtQuantServer | xtdata（标准模式受 BSON crash 限制，降级 Tushare/Mootdx） |
| XtQuantManager | `ENABLE_XTQUANT_MANAGER` | HTTP 网关 | 只读监控 |
| 文件 IPC | `ENABLE_QMT_IPC_FALLBACK` | 文件系统 `C:\QuantIPC` | ❌ 不覆盖 |
| **RPC（本方案）** | **`ENABLE_QMT_RPC_FALLBACK`** | **Redis/ZMQ/MySQL** | **✅ get_full_tick / get_market_data / L2 / 财务 等 117 个** |

**本方案的增量价值**：
1. **交易通道更快、可跨机**：Redis 13ms vs 文件轮询 1s；多账号可集中到一台 Redis。
2. **行情能力**：外部库的 `xtdata` RPC 把 `get_market_data_ex` 等调用隔离在大QMT进程内，
...
| P4 | ✅ 完成 | [test/test_qmt_rpc_trader.py](../test/test_qmt_rpc_trader.py) 13 用例通过；`qmt_rpc` 测试组；`--fast` 702 用例 100% 无回归 |
| P5 | ⏳ 待做 | 需在装大QMT机器上跑 `BIGQMT_REDIS_DRYRUN.py` 联调 |
| P6 | 🔄 进行中 | 本文档已更新；CLAUDE.md 待补 |
...
> 交易通道（P1–P6）稳定后的独立子项，与交易开关**正交**，单独开关
> `ENABLE_QMT_RPC_DATA_SOURCE`。核心价值：用真·QMT 实时行情替代当前标准模式的
