...
项目测试代码位于 [test/](https://github.com/weihong-su/miniQMT/tree/main/test) 目录，使用标准 `unittest`。当前回归配置见 `test/integration_test_config.json`，包含 35 个测试组（含 `fast` 快速子集）。

最近一次（2026-09-12, v3.9.1）使用 Anaconda `python39` 执行 `--all-with-fast` 实测：**36 组、146 模块、3126 用例、3126 通过、0 失败、0 错误、0 跳过，成功率 100%**。
...
| v3.9.0 | 2026-08-29 | 35 组, 130 模块, 2622 用例 | 2622 | 100% |
| v3.8.9 | 2026-08-19 | 33 组, 123 模块, 2548 用例 | 2548 | 100% |
| v3.8.8 | 2026-08-13 | 33 组, 123 模块, 2479 用例 | 2479 | 100% |
| v3.8.7 | 2026-08-10 | 33 组, 123 模块, 2453 用例 | 2453 | 100% |
| v3.8.6 | 2026-08-07 | 33 组, 123 模块, 2361 用例 | 2361 | 100% |
...
`order_stock_async()` 的 `seq -> order_id` 行为已经固化为单元/集成回归，不再保留可触发实盘下单的 Web 调试窄接口。相关实盘行为结论和匹配原则见 [QMT order_id 匹配](qmt-order-id-matching.md)。
...
| `fast` | critical | 5 分钟快速验证子集（当前 40 个模块、959 个用例） |
