...
!!! tip "对照后台日志排查时的 ID 术语  [2026-09-17]"
    把 Web 上的一笔委托和后台日志对上时，认准 **`委托号=`**（即 `order_id`，贯穿下单→回调→成交→落库的主键）。注意日志里还有几个形近但不同的值：`请求序号=`（异步下单的本地临时凭据，不能用于撤单）、`柜台编号=`（交易所侧编号，旧称「系统订单号」）、`成交编号=`（真实成交号）、`流水号=`（`trade_records` 落库主键）。

    网格成交的 `流水号=` 与 `委托号=` 数值相同是**设计如此**（网格落库时用委托号充当流水号），不是重复记账。完整术语表见 [CLAUDE.md](https://github.com/Su-M10/miniQMT/blob/main/CLAUDE.md) 开发规范一节。
