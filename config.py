...
# 盘中今日日线通常未稳定落库，默认收盘后一段时间才把今天作为历史日线截止日
HISTORY_TODAY_DAILY_AVAILABLE_AFTER = "15:30:00"
# 历史数据为空/指标数据为空时的告警降噪间隔（秒）
HISTORY_NO_DATA_LOG_INTERVAL_SECONDS = 300
INDICATOR_EMPTY_DATA_LOG_INTERVAL_SECONDS = 300
# 备选池股票文件路径
...
ENABLE_PAUSE_GRID_AFTER_TAKE_PROFIT_FULL = _env_bool("ENABLE_PAUSE_GRID_AFTER_TAKE_PROFIT_FULL", True)  # 全仓止盈/止损成交后暂停该股网格会话(仅置 enabled=False，不改网格配置)
...
# MACD 操盘建议(悬浮窗)配置
SHENZHEN_INDEX_CODE = "399001.SZ"   # 账号级建议使用的深证成指代码
MACD_ADVICE_CACHE_TTL = 300         # 建议结果缓存时长(秒)，日线日内基本不变
